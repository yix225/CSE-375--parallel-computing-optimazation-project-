...
class concurrent {
    private:
        ...
        bool relocate(int index, int h_index){
            ...
            for (int round = 0; round < limit; round++){
                ...
                if (success){
                    if (tables[index1][h_index1].size()< thredhold){
                ...
                }else if (tables[index][h_index].size() >= thresdhold){
                    release(value);
                    continue;
                }else{
                    release(value);
    ...
    public:
        concurrent(int capacity, int limit, int num_locks, int thredhold, int pro_size){
            this->capacity = capacity;
            this->limit = limit;
            this->locks = num_locks;
            this->threshold = thredhold;
        ...
        bool add(T value){
            ...
            if (tables[0][index0].size() < thredhold){
                tables[0][index0].push_back(value);
                release(value);
                return true;
            }else if (tables[1][index1].size() < thredhold){
