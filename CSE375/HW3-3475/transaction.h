...
class transaction{
    private: 
        ...
        void resizeTables(int oldCapacity){
            std::vector<std::vector<T*>> tmp_tables[2] = tables;
            ...
            for (int round = 0; round < capacity; round++){
                ...
                _transaction_atomic{
                    ...
                    if(prob_remove(value, temp)){
                        if(prob_size(value1) < threshold){
                            if(prob_add(value1, temp)){
                                return true;
                            }
                        }else if(prob_size(value1)< prob_size){
                            if(prob_add(value1, temp)){
                                index = 1 - index;
                                h_index = h_index1;
                                index1 = 1 - index;
                    ...
                    }else if(prob_size(value) >= threshold){
                        continue;
                    } else{
                        return true;
                    } 
        ...
        public:
        //n = 1000, int limit = 10, int threshold = 2, int probe_size = 4
        transaction(int n = 1000, int limit = 10, int threshold = 2, int probe_size = 4) : N(n), limit(limit), threshold(threshold), prob_size(probe_size){
            this->N = n;
            this->limit = limit;
            this->threshold = threshold;
            this->prob_size = probe_size;
            capacity = n;
            isResize = true;
        ...
        bool add(T x){
            ...
            __transaction_atomic{
                ...
                if(prob_size(tables[0][h0]) < threshold){
                    if(prob_add(tables[0][h0], x)){
                        return true;
                    }
                }else if(prob_size(tables[1][h1]) < threshold){
                    if(prob_add(tables[1][h1], x)){
                        return true;
                    }
                }else if (prob_size(tables[0][h0]) < prob_size){
                    if(prob_add(tables[0][h0], x)){
                        i = 0;
                        h = h0;
                    }
                }else if(prob_size(tables[1][h1]) < prob_size){
                    if(prob_add(tables[1][h1], x)){
                        i = 1;
                        h = h1;
                    }
        ...
        int size(){
            ...
            for(int i = 0; i < capacity; i++){
                count += prob_size(tables[0][i]);
                count += prob_size(tables[1][i]);
            }
            return count;
        }
